import { X, Y } from '../../../latex2js-utils/src/index.js';
import * as d3 from '../../modules/d3/index.js';
...
const psgraph = {
  ...
  psaxes(svg) {
    ...
    var xticks = function (ob) {
      // draw ticks
      //for (var x = xaxis[0]; x <= xaxis[1]; x += this.dx) {
      for (var x = xaxis[0]; x <= xaxis[1]; x += ob.dx) {
        line(x, origin[1] - 5, x, origin[1] + 5);
      }
    };

    var yticks = function (ob) {
      // draw ticks
      //for (var y = yaxis[0]; y <= yaxis[1]; y += this.dy) {
      for (var y = yaxis[0]; y <= yaxis[1]; y += ob.dy) {
  ...
  pspicture(svg) {
    ...
    svg.on(
      'touchmove',
      function (that) {
        //d3.event.preventDefault(); //for d3 v4.13.0
        //var touchcoords = d3.touches(this)[0];   //for d3 v4.13.0
        var touchcoords = d3.pointers(that)[0];  //for d3 v6++
    ...
    svg.on(
      'mousemove',
      function (that) {
        //var coords = d3.mouse(this);   //for d3 v4.13.0
        var coords = d3.pointer(that); //for d3 v6++
        userEvent(coords);
      },
      false
    );
